...
- **Item 9**: modo oscuro -- HECHO (12/09/2026). Tokens de color en `css/style.css` (fondo, superficies, bordes, texto, badges de estado, botones de peligro/éxito, insignias de plataforma) con override oscuro vía `prefers-color-scheme` + `data-theme` en `<html>`; botón "Sistema/Claro/Oscuro" en la barra lateral (`js/theme.js`, guarda la elección en `localStorage.hub_theme`). Los mockups de redes (feed IG, muro FB, post LI, stories) quedan siempre con su apariencia real, no cambian con el tema. Pendiente de un repaso: badges/colores sueltos hardcodeados directo en `style="..."` inline dentro de `admin/index.html` y `js/app.js` (hay muchos, no se tocaron en esta tanda) pueden verse desentonados sobre fondo oscuro -- corregir a medida que se detecten.
