...
- **SIEMPRE optimizar el responsive al codear, en TODO lo que se toque.** Pedido explícito y enfático de Vaneh (03/09): "la versión mobile es la más importante de hecho". No es solo para features nuevas — cualquier UI que se agregue o edite (botones, filtros, modales, toolbars) tiene que probarse mentalmente en mobile antes de darla por terminada: usar `flex-wrap:wrap`, anchos `auto`/`%` en vez de fijos, y si una fila de controles crece mucho pensar en compactarla (ver pendiente de "menú de filtros que se asoma por la derecha en mobile" en este HANDOFF).
